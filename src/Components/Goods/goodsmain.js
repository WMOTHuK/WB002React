...
async function getadddatagoods(finalData, setStatus) {
  setStatus(prevStatus => [...prevStatus, `Начало обработки данных из таблицы photos`]);
  // Получение данных из таблицы photo
  const photoDataFromDB = await gettablefrombd('photos');
  setStatus(prevStatus => [...prevStatus, `Данные из таблицы photos успешно получены`]);
...
function Goodsmain() {
  ...
  const fetchData = async () => {
    setStatus(prevStatus => [...prevStatus, `Начало загрузки данных из базы данных`]); 
    setGdata(null);
    settrans([]);
    try {
      const finaldata = await gettablefrombd('goods');
      setStatus(prevStatus => [...prevStatus, `Данные из таблицы goods успешно получены`]);

        // Удаляем записи, у которых deleted = true
      const filteredData = finaldata.filter(item => item.deleted !== "X");
      setStatus(prevStatus => [...prevStatus, `Удалены записи с deleted = true`]);
      ...
      if (filteredData) {
          ...
          setStatus(prevStatus => [...prevStatus, `Данные из таблицы goods отсортированы по vendorcode`]);
        resultData = await getadddatagoods(filteredData, setStatus);
        setGdata(resultData);
        setStatus(prevStatus => [...prevStatus, `Данные товаров успешно обработаны и установлены в состояние`]);
      ...
      setStatus(prevStatus => [...prevStatus, `Ключи таблицы успешно получены`]);
      const translations = await gettablelocale(tablekeys, globalData.locale);
      setStatus(prevStatus => [...prevStatus, `Переводы для таблицы успешно загружены`]);
      settrans(translations);
    } catch (error) {
      setStatus(prevStatus => [...prevStatus, `Ошибка при загрузке данных(fetchdata): ${error.message}`]);
      setGdata(null);
      setIsLoading(false); 
    }
  };
